                 ...
                 COLOR Color_Background, COLOR Color_Foreground)
{
    ...
    for (Page = 0; Page < Font->Height; Page++)
    {
        for (Column = 0; Column < Font->Width; Column++)
        {
            ...
            { // this process is to speed up the
              // scan
                        ...
                        COLOR Color_Background, COLOR Color_Foreground)
{
    ...
    while (*pString != '\0')
    {
        // Start new line if X direction is already filled or it is
        ...
        if ((Xpoint + Font->Width) > Xend || *pString == '\n')
        {
            ...
            // If next character is space, just skip it on the
            // beginning of the new line or in case of special
            // character
            if (*pString == ' ' || *pString == '\n')
                ++pString;

            // After some skipping chack again if it's not '\0'
            // character
            if (*pString == '\0')
                break;
        }
                   ...
                   int data_size)
{
    ...
    for (yDirNum = 0; yDirNum < sLCD_DIS.LCD_Dis_Page; ++yDirNum)
    {
        ...
        {
            if (data_size < xDirNum * yDirNum * 3 + 2)
                return;

            // Conversion to RGB565
            int8_t r = image_data[xDirNum * yDirNum * 3];
            int8_t g = image_data[xDirNum * yDirNum * 3 + 1];
            int8_t b = image_data[xDirNum * yDirNum * 3 + 2];
            COLOR color =
                ((r & 0b11111000) << 8) | ((g & 0b11111100) << 3) | (b >> 3);
            LCD_SetPointlColor(xPoint + xDirNum, yPoint + yDirNum, color);
        }
    }
}
...
/******************************************************************************
 ...
 parameter:
                 t	:   textbox where text should be refreshed or
 displayed
 ******************************************************************************/
...
/******************************************************************************
 function:	Adds new first line with provided text, moving older
 text to the next ones. parameter: c		:   console where the
 next line of text should be added text	:	text to add on the
 beggining
 ******************************************************************************/
...
/******************************************************************************
 function:	Adds new first line with provided formatted text (like
 printf), moving older text to the next ones. parameter: c
 :   console where the next line of text should be added fmt	:
 formatted text to add on the beginning
 ******************************************************************************/
